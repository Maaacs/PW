(function () {

  const FPS = 300;
  const HEIGHT = 300;
  const WIDTH = 1024;
  const PROB_NUVEM = 1;

  const NIVEL_1 = 500;
  const NIVEL_2 = 450;
  const NIVEL_3 = 400;
  const NIVEL_4 = 300;
  const LIMITE_TELA = '1024px';
  const VELOCIDADE = 3; // Do jogo (Deserto, Obstaculos, Pterodactilo)

  let gameLoop;
  let deserto;
  let dino;
  var nuvens = [];
  let frame = 0;

  var scoreLoop;
  var pontuacao = [0, 0, 0, 0, 0];
  var pontuacaohi = [0, 0, 0, 0, 0];
  var obstaculos = [];
  let paused = false;
  let over = false;
  let tipo = '';
  let obstaculosExibir = false;
  let contador = 0;
  let nivel = 0;


  const pontuacaoPx = {
    0: '-970px',
    1: '-986px',
    2: '-999px',
    3: '-1015px',
    4: '-1029px',
    5: '-1044px',
    6: '-1059px',
    7: '-1075px',
    8: '-1089px',
    9: '-1105px',
    'H': '-1120px',
    'I': '-1136px',
  };

  const cactusPequenosPx = {
    1: ['-335px', '-360px', '-386px', '-412px', '-437px', '-463px'],
    2: ['-335px', '-360px', '-386px', '-412px', '-437px'],
    3: ['-335px', '-360px', '-386px', '-412px'],
    4: ['-335px', '-360px', '-386px'],
    'width': { // grupo de cactus
      1: '23px',
      2: '50px',
      3: '75px',
      4: '100px',
    }
  };

  const cactusGrandesPx = {
    1: ['-490px', '-527px', '-565px', '-602px', '-681px'],
    2: ['-490px', '-527px', '-565px', '-638px'],
    3: ['-490px', '-527px', '-565px', '-602px'],
    4: ['-490px'],
    'width': { // grupo de cactus
      1: '35px',
      2: '71px',
      3: '110px',
      4: '146px',
    },
  };


  const alturaPterodactilo = ['170px', '190px', '240px'];

 
  function init() {
    deserto = new Deserto(document);
    dino = new Dino(deserto, document);
    pontuacao = new Pontuacao(); 
    pontuacao.listarDigitos();
    pontuacao.adicionarPontos();
    pontuacaohi = new PontuacaoHI(pontuacao); // Passa a instância de Pontuacao para PontuacaoHI
    pontuacaohi.mostrarHighscore();
  
    window.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowUp' || e.code === 'Space') {
        if (dino.status === -1 && !over){
          dino.status = 0;
          const intervalo = 1000 / FPS; // Intervalo de tempo 
          gameLoop = setInterval(run, intervalo);
          turno = setInterval(noiteDia, 60 * 1000);
          scoreLoop = setInterval(function () {
            if (!paused && !over) {
              pontuacao.incrementar(document);
              pontuacao.listarDigitos();
            }
          }, 1000 / 30);
        }
      }
    });
  }

  window.addEventListener("keydown", (e) => {
    if (e.repeat) {
      return;
    }
    if (e.code === 'Space' || e.key === 'ArrowUp'){
      if(dino.status === 0)dino.status = 1;
    } 
    if (e.key === 'ArrowDown'){
      if (dino.status === 0) dino.status = 3;
    }
    if (e.key === 'p') togglePause();
  });

  window.addEventListener("keyup", (e) => { // Dino levantar
    if (e.key === 'ArrowDown' && dino.status === 3){
      if (dino.status === 3) dino.status = 0;
    } 
  })

  

  class Deserto {
    constructor() {
      this.element = document.createElement("div")
      this.element.className = "deserto";
      this.element.style.width = `${WIDTH}px`;
      this.element.style.height = `${HEIGHT}px`;
      document.getElementById("game").appendChild(this.element)

      this.chao = document.createElement("div")
      this.chao.className = "chao"
      this.chao.style.backgroundPositionX = 0;
      this.element.appendChild(this.chao)
    }
    mover() {
      this.chao.style.backgroundPositionX = `${parseInt(this.chao.style.backgroundPositionX) - VELOCIDADE}px`
    }
  }


  class Dino {
    #status
    constructor() {
      this.backgroundPositionsX = {
        'correndo1': "-1391px",
        'correndo2': "-1457px",
        'pulando': "-1259px",
        'agachado1': '-1653px',
        'agachado2': '-1742px',
        'parado': '-56px' 
      };
      this.#status = -1; // 0:correndo; 1:subindo; 2: descendo; 3: agachado; -1: esperando
      this.alturaMinima = 2;
      this.alturaMaxima = 180;
      this.element = document.createElement('div');
      this.element.className = 'dino';
      this.element.style.backgroundPositionX = this.backgroundPositionsX.parado;
      this.element.style.backgroundPositionY = "-2px";
      this.element.style.bottom = `${this.alturaMinima}px`
      deserto.element.appendChild(this.element);
    }
    /**
     * @param {number} value
     */
    set status(value) {
      if (value >= 0 && value <= 5) this.#status = value;
    }
    get status() {
      return this.#status;
    }
    correr() {
      if (this.#status == 0 && frame % 20 === 0){
      this.element.style.width = '67px';
      this.element.style.height = '71px';
      this.element.style.backgroundPositionY = '-4px';
      this.element.style.backgroundPositionX = this.element.style.backgroundPositionX === this.backgroundPositionsX.correndo1 ? this.backgroundPositionsX.correndo2 : this.backgroundPositionsX.correndo1;
      }else if (this.#status == 1) {
        this.element.style.backgroundPositionX = this.backgroundPositionsX.pulando;
        this.element.style.bottom = `${parseInt(this.element.style.bottom) + 2}px`;
        if (parseInt(this.element.style.bottom) >= this.alturaMaxima) this.status = 2;
      }
      else if (this.#status == 2) {
        this.element.style.bottom = `${parseInt(this.element.style.bottom) - 2}px`;
        if (parseInt(this.element.style.bottom) <= this.alturaMinima) this.status = 0;
      } 
      else if (this.#status == 3) { 
        this.element.style.width = '87px';
        this.element.style.height = '47px';
        this.element.style.backgroundPositionY = '-22px'; 
        this.element.style.backgroundPositionX = this.element.style.backgroundPositionX === this.backgroundPositionsX.agachado1 ? this.backgroundPositionsX.agachado2 : this.backgroundPositionsX.agachado1; 
      }    
    }
  }
  

  class Nuvem {
    constructor(document) {
      this.element = document.createElement('div');
      this.element.className = 'nuvem';
      this.element.style.right = 0;
      this.element.style.top = `${parseInt(Math.random() * 200)}px`
      deserto.element.appendChild(this.element);
    }

    mover() {
      this.element.style.right = (parseInt(this.element.style.right) + 1) + 'px';
    }

    remover() {
      this.element.parentNode.removeChild(this.element);
    }
  }

  class Obstaculo {
    constructor(document) {
      this.tipo = (Math.floor(Math.random() * (+3 - +1)) + +1 === 1) ? 'cactus' : 'peterodactilo';
    }

    mover() {
      this.element.style.right = (parseInt(this.element.style.right) + VELOCIDADE) + 'px';
    }

    remover() {
      this.element.parentNode.removeChild(this.element);
    }
  }

  class Cactus extends Obstaculo {
    constructor(document) {
      var index;
      super();
      // quantidade de cactus por nível
      this.tipo = 'cactus';
      if (nivel === 0) {
        this.quantidade = 1;
      }else if ( nivel <= 20) {
        this.quantidade = 1;  
      }else if ( nivel>20 && nivel<=40) {
        this.quantidade = 2;  
      }else if ( nivel>40 && nivel<=80) {
        this.quantidade = 3; 
      }  
      else {
        this.quantidade = 3; // 4 é impossível de pular haha
      }
      this.tamanho = (Math.floor(Math.random() * (+3 - +1)) + +1 === 1) ? 'grande' : 'pequeno';
      if (this.quantidade >= 0) {
        this.element = document.createElement('div');
        this.element.style.right = '-50px';
        this.element.style.bottom = '-22px';
        if (this.tamanho === 'pequeno') {
          index = Math.floor(Math.random() * cactusPequenosPx[this.quantidade].length);
          this.element.className = 'cactus pequeno';
          this.element.style.backgroundPositionX = cactusPequenosPx[this.quantidade][index];
          this.element.style.width = cactusPequenosPx.width[this.quantidade];
          deserto.element.appendChild(this.element);
        } else if (this.tamanho === 'grande') {
          index = Math.floor(Math.random() * cactusGrandesPx[this.quantidade].length);
          this.element.className = 'cactus grande';
          this.element.style.backgroundPositionX = cactusGrandesPx[this.quantidade][index];
          this.element.style.width = cactusGrandesPx.width[this.quantidade];
          deserto.element.appendChild(this.element);
        }
      }
    }
  }

  class Pterodactilo extends Obstaculo {
    constructor(document) {
      super();
      var index = Math.floor(Math.random() * 3); 
      this.baixo = true;
      this.tipo = 'pterodactilo';
      this.element = document.createElement('div');
      this.element.className = 'pterodactilo';
      this.element.style.right = '-50px';
      this.element.style.top = alturaPterodactilo[index];
      this.element.style.backgroundPositionX = '-196px';
      this.element.style.backgroundPositionY = '-13px';
      this.baixoInterval = 0;
      deserto.element.appendChild(this.element);
    }

    mover() {
      this.element.style.right = (parseInt(this.element.style.right) + VELOCIDADE) + 'px';
      // atraso entre o bater das asas
      if (this.baixoInterval >= 14) {
        if (this.baixo) {
          this.element.style.backgroundPositionX = '-266px';
          this.element.style.backgroundPositionY = '-3px';
          this.element.style.height = '39px';
          this.baixo = false;
        } else {
          this.element.style.backgroundPositionX = '-196px';
          this.element.style.backgroundPositionY = '-13px';
          this.element.style.height = '45px';
          this.baixo = true;
        }
        this.baixoInterval = 0;
      } else {
        this.baixoInterval++;
      }
    }
  }


  class Digito {
    constructor(item, index) {
      this.valor = 0;
      this.element = document.createElement('div');
      this.element.className = 'digito';
      this.element.id = `${index}`;
      this.element.style.backgroundPositionY = '0px';
      this.element.style.backgroundPositionX = pontuacaoPx[item];
    }
    atualizar(item) {
      this.element.style.backgroundPositionX = pontuacaoPx[item];
    }
  }

  class Pontuacao {
    constructor() {
      this.valor = 0;
      this.element = document.createElement('div');
      this.element.className = 'pontuacao';
      deserto.element.appendChild(this.element);
    }
  
    zerar() {
      this.valor = 0;
    }
  
    listarDigitos() {
      this.lista = [];
      let pontos = this.valor.toString(10).split('');
      pontos.forEach(ponto => {
        this.lista.push(parseInt(ponto));
      });
  
      while (this.lista.length !== 5) {
        this.lista.unshift(0);
      }
      return this.lista;
    }
  
    adicionarPontos() {
      let index = 0;
      this.lista.forEach(item => {
        const ponto = new Digito(item, index);
        index = index + 1;
        this.element.appendChild(ponto.element);
      });
    }
  
    incrementar(document) {
      this.valor = this.valor + 1;
      this.listarDigitos();
      for (let i = 0; i < this.lista.length; i = i + 1) {
        const elemento = document.getElementById(`${i}`);
        elemento.style.backgroundPositionX = pontuacaoPx[this.lista[i]];
      }
    }
  }
  
  class PontuacaoHI extends Pontuacao {
    constructor(pontuacao) {
      super();
      this.highscore = localStorage.getItem('highscore') || 0; // Lê o highscore do armazenamento local
      this.pontuacao = pontuacao; // Compartilha a mesma instância de Pontuacao
      this.elementHI = document.createElement('div');
      this.elementHI.className = 'pontuacaohi';
      deserto.element.appendChild(this.elementHI);

      this.elementHI2 = document.createElement('div');
      this.elementHI2.className = 'hi';
      
      this.elementHI2.style.backgroundPositionY = '0px';
      this.elementHI2.style.backgroundPositionX = '-1120px';
      deserto.element.appendChild(this.elementHI2);

      this.mostrarHighscore();
    }
  
    verificarHighscore() {
      if (this.pontuacao.valor > this.highscore) {
        this.highscore = this.pontuacao.valor;
        this.mostrarHighscore();
        // Atualiza o highscore no armazenamento local
        localStorage.setItem('highscore', this.highscore);
      }
    }
  
    mostrarHighscore() {
      let indexHI = 5;
      let highscoreLista = [];
      let highscorePontos = this.highscore.toString(10).split('');
      highscorePontos.forEach(ponto => {
        highscoreLista.push(parseInt(ponto));
      });
  
      while (highscoreLista.length !== 5) {
        highscoreLista.unshift(0);
      }
  
      this.elementHI.innerHTML = '';
  
      highscoreLista.forEach(item => {
        const pontoHI = new Digito(item, indexHI);
        indexHI = indexHI + 1;
        this.elementHI.appendChild(pontoHI.element);
      });
    }
  }
  

  function noiteDia() {
    document.body.style.backgroundColor = document.body.style.backgroundColor === 'black' ?
      'white': 'black';
    document.body.style.wbebkitFilter = document.body.style.wbebkitFilter === 'invert(1)' ? 
      'none' : 'invert(1)';
    document.body.style.filter = document.body.style.filter === 'invert(1)' ? 
      'none' : 'invert(1)';
  }

  function togglePause() {
    if (!paused) paused = true;
    else if (paused) paused = false;
  }

  function escolheObstaculo(document) {
    tipo = (Math.floor(Math.random() * (+3 - +1)) + +1 === 1) ? 'cactus' : 'pterodactilo';
    if (tipo === 'cactus') {
      obstaculos.push(new Cactus(document));
    } else if (tipo === 'pterodactilo') {
      obstaculos.push(new Pterodactilo(document));
    }
  }
  
  function colisao(obs) {
    const dinox = dino.element.offsetLeft;
    const dinoy = dino.element.offsetTop; 
    const dinow = dino.element.offsetWidth - 10;
    const dinoh = dino.element.offsetHeight - 5;

    const obsx = obs.element.offsetLeft;
    const obsw = obs.element.offsetWidth - 10;
    const obsh = obs.element.offsetHeight;
    const obsy = obs.element.offsetTop + 10;
    
    const colisao1 = (obsx + obsw >= dinox) && (dinox + dinow >= obsx);
    const colisao2 = (obsy + obsh >= dinoy) && (dinoy + dinoh >= obsy);

    return colisao1 && colisao2;
  }



  function endgame() {
    const element = document.createElement('div');
    element.className = 'endgame';
    element.over = document.createElement('div');
    element.over.className = 'over';
    document.querySelector('.over').style.display = 'block';
    element.restart = document.createElement('button');
    element.restart.className = 'restart';
    document.querySelector('.restart').style.display = 'block';
    dino.element.style.backgroundPositionX = '-1526px';
    dino.element.style.backgroundPositionY = '-4px';
    dino.element.style.width = '60px';
    dino.element.style.height = '66px';
    element.appendChild(element.over);
    element.appendChild(element.restart);
    document.body.appendChild(element);
    pontuacaohi.verificarHighscore();
    // pontuacaohi.mostrarHighscore();

    element.restart.addEventListener('click', function() {
      document.location.href = '';
    });
    window.addEventListener('keydown', function (e) {
      if (e.key === 'ArrowUp' || e.code === 'Space') document.location.href = '';
    });
  }


  function run() {
    frame = frame + 1
    contador = contador + 1
   
    // Disposição dos obstaculos na tela
    if (nivel <= 1 && contador >= NIVEL_1) {
      obstaculosExibir = true;
      contador = 0;
      nivel += 1;
      console.log('Entrou no primeiro if');
    }
    else if (nivel == 2 && contador >= NIVEL_2) {
      obstaculosExibir = true;
      contador = 0;
      nivel += 1;
      console.log('Entrou no segundo if');
    }
    else if (nivel == 3 && contador >= NIVEL_3){
      obstaculosExibir = true;
      contador = 0;
      nivel += 1;
      console.log('Entrou no terceiro if');
    } 
    else if (nivel >= 4 && contador >= NIVEL_4){
      obstaculosExibir = true;
      contador = 0;
      nivel += 1;
      console.log('Entrou no quarto if. Nivel', nivel);
    } 

    if (!paused) {

      if (frame === FPS) frame = 0;
      deserto.mover();
      dino.correr();
      if (Math.random() * 100 <= PROB_NUVEM) nuvens.push(new Nuvem(document));
      if (frame % 2 === 0){
        nuvens.forEach(nuvem => nuvem.mover());
        if (nuvens.element.style.right === LIMITE_TELA) n.remover();
      } 
        
      if (obstaculosExibir) {
        escolheObstaculo(document);
        obstaculosExibir = false;
      }
      obstaculos.forEach(function (o) {
        o.mover();
        if (o.element.style.right === LIMITE_TELA) o.remover();
        if (colisao(o)) {
          over = true;
          endgame();
          clearInterval(gameLoop);
          clearInterval(turno);
          clearInterval(scoreLoop);
        };
      });
    }
  }
  
  
  init();

})();